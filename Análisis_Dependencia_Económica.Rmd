...
```{r}
# Cargar dataset
url_raw <- "https://raw.githubusercontent.com/Edgardo2501/Data_Analysis_Projects/refs/heads/main/dependencia_economica.csv"

data_dependencia_economica <- read.csv(url_raw)
```
...
data_dependencia_economica %>% 
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 1), 
        legend.position = "") +
  scale_fill_manual(values = c("Si" = "mediumorchid4", "No" = "#27408B"))
```
...
data_dependencia_economica %>% 
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 1), 
        legend.position = "") +
  scale_fill_manual(values = c("Rural" = "#A0522D", "Urbana" = "#36648B"))
```
...
trabajo_nr_por_sexo <- data_dependencia_economica %>% 
  group_by(sexo) %>% 
...
trabajo_nr_por_etnia <- data_dependencia_economica %>% 
  group_by(etnia, sexo) %>% 
  summarise(medialog_tnr = mean(horas_log), .groups = 'drop')
...
trabajo_nr_por_afrod <- data_dependencia_economica %>% 
  group_by(afrod, sexo) %>% 
  summarise(medialog_tnr = mean(horas_log), .groups = 'drop')
...
trabajo_nr_por_zona <- data_dependencia_economica %>% 
  group_by(zona, sexo) %>% 
  summarise(medialog_tnr = mean(horas_log), .groups = 'drop')
...
trabajo_nr_por_disc <- data_dependencia_economica %>% 
...
# trabajo_nr_por_reg <- data_dependencia_economica %>% 
