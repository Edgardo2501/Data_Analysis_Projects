...
```{r include=FALSE}
# Cargar dataset
# 1. Definir el ID del archivo (extraído del enlace original)
file_id <- "1ZqzPggpu9-BX1aat7DOLdKgxPFKF0MHo"

# 2. Construir la URL de descarga directa
url_raw <- paste0("https://drive.google.com/uc?id=", file_id, "&export=download")
...
ggplot(trabajo_nr_por_sexo, aes(x = sexo, y = media_hm, fill = sexo)) +
  ...
  geom_text(aes(label = round(media_tnr, 2),vjust = -0.5, size = 1)) +
...
ggplot(trabajo_nr_por_etnia, aes(x = sexo, y = media_hm, fill = etnia)) +
